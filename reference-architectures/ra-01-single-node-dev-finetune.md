...
要点:单机没有"调度器选型"问题——不必上 Slurm 或 K8s,一个卡位登记 + 排队约定(哪怕是一个共享表格加环境变量纪律)就够;真正不能省的是**单卡粒度的配额与故障卡隔离**([§8.4.6](../第二部分-算力底座/第08章-K8s上的加速卡.md#846-节点生命周期驱动健康检查与自动隔离) 的单机版)。
...
- 激活按 F5 选择性重计算仍达 ≈ 91 GB/卡(L=80,h=8192,s=4096,b=1),必须开完全重计算压到个位数 GB,代价是 F1 的系数 6 推向 7~8([§17.3.3](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#1733-激活重计算用算力换显存的三档粒度));
- 合计 ≈ 25~30 GB/卡 < 63 GB,**8 卡可行**;QLoRA(权重 0.5N = 35 GB)是单卡跑 70B 的备选,代价见 [§17.4](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#174-方案对比显存不够的三条路线)。
...
- **单卡故障域**:XID/ECC 故障按 [§8.4.6](../第二部分-算力底座/第08章-K8s上的加速卡.md#846-节点生命周期驱动健康检查与自动隔离) 的思路做健康检查与隔离——坏一张卡,平台从 8 卡降级为 7 卡继续服务,而不是全机排障。
- **任务级故障**:是否采用人工恢复，按本机任务影响事件率、值班响应、checkpoint pause/回滚和结果 SLO 决定；卡数与任务时长只进入 [§20.4.1](../第四部分-训练系统/第20章-稳定性与容错.md#2041-失败率模型故障为什么是常态) 的风险情景，不能单独证明手工重启足够。周期 checkpoint 的间隔用本地事件与恢复演练复算。
...
单机不必上完整监控栈,但三类信号必须有,口径与 [第 29 章](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md) 对齐,规模化后可平移:
