...
- 先从任务事件日志按共同时间窗与故障域去重，估计会让该任务回滚的事件间隔 `M`；不得把示意单卡 MTBF 直接除以 1024，因为共享故障和多源重复告警会破坏独立性([§20.4.1](../第四部分-训练系统/第20章-稳定性与容错.md#2041-失败率模型故障为什么是常态))；
- 对周期同步保存，可用 `τ≈√(2CM)` 初筛，其中 `C` 是训练实际 pause；异步 staging、检测延迟、分层恢复与 RPO 另行约束；
- A 使用互斥时间账实测：checkpoint 阻塞、检测、隔离、资源等待、初始化、加载、回放、失败重算与慢节点拖累分别计时。容量规划可做低/中/高情景，但 `A=0.90` 只能作为声明假设，不能声称由固定 MTBF 和通用百分比推导得出([§20.4.2](../第四部分-训练系统/第20章-稳定性与容错.md#2042-有效训练时间占比稳定性的唯一北极星指标))。
...
**checkpoint 带宽容量示例:** 若 70B 任务确实保存 16 B/param 的训练态，数据量约 1.12 TB；用 25 GB/s 聚合带宽相除只能得到约 45 s 的数据段下界。排队、元数据、最慢 rank、commit 与 verify 另测，保存间隔还受 RPO 和恢复成本约束。是否采用节点暂存分层，要验证暂存故障域、后台积压和目标规模 restore，而不是由千卡标签自动决定([§14.4.3](../第三部分-数据底座/第14章-存储与Checkpoint-IO.md#1443-checkpoint-的突发写模式本章核心)、[§14.5.1](../第三部分-数据底座/第14章-存储与Checkpoint-IO.md#1451-三种-checkpoint-优化))。

**并行与互联域对齐(§16.4.5/§20.4.5 的联合约束):** 并行选型先用 F4/F5 定切分、F15–F20 核通信。高频 TP/EP/CP 组优先放在高速域内；若容量迫使其跨域，须把实际进程组、跨域字节、collective 暴露时间和受控短跑结果列为验收证据，而不是用“千卡”标签直接判定。互联域扩大还会放大共同失效范围，热备、checkpoint 与重启粒度应按目标故障域验证([§20.4.5](../第四部分-训练系统/第20章-稳定性与容错.md#2045-故障域的放大超节点改写全部容错算术重点))。
...
北极星是**有效训练时间占比**,一切指标为它服务(口径与 [第 29 章](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md)、[第 31 章](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#310-两条主线最终汇入同一本账) 对齐):
...
- **电费与散热进入主成本**:千卡级功率以兆瓦计,PUE 每降 0.1 都是实钱,液冷与机房方案在采购阶段定型([§31.2.4](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3124-pue-与电费功率如何进入-tco));能耗与碳账口径见 [第 31 章「从电费到碳账」](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3134-从电费到碳账功能单位与系统边界);
