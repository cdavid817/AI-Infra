...
- 端到端可观测:一次任务是一棵标准嵌套 span 树,超时预算逐层有实测依据([§29 OTel GenAI](../第六部分-上层平台/第29章-可观测性与评测流水线.md#2923-把指标说同一种语言otel-genai-语义约定))。
...
- **质量维度**:Agent 任务的"不报错但变笨"同样存在(模型换版本后工具调用成功率、任务完成率漂移),接[§29.2.5](../第六部分-上层平台/第29章-可观测性与评测流水线.md#2925-评测是一个吃卡的批任务) 的评测流水线做回归。
