...
- 队列化多租共享:配额、优先级、抢占、Gang 语义齐备([§9.4.3](../第二部分-算力底座/第09章-批调度器.md#943-队列配额优先级抢占四件套的模型设计));
...
- 通信量核对(F15/F17):先确认目标 TP 图中的 AllReduce/ReduceScatter/AllGather 事件与 payload，再验证 TP 组完整落在 8 卡高速域内。跨机 DP bucket 是否被反向隐藏由 trace 判定，不能由参数量估算直接推出“不压 MFU”；方法见 [§16.4.2](../第四部分-训练系统/第16章-并行策略.md#1642-通信量与显存收益每种策略的账)。

**checkpoint 窗口(第 14 章口径):**

- 13B 训练态快照 ≈ 16N = 208 GB;假设聚合客户端带宽 100 Gbps(12.5 GB/s),同步直写停顿 ≈ 17 s;
- 本容量示例暂以单卡 MTBF 5×10^4 h、独立故障和同步周期保存代入 Young 一阶近似；上线前必须用集群级故障、实际 checkpoint pause、RPO/RTO 与恢复演练替换。同步写是否够用由训练和共享存储 SLO 决定，不能只按 128 卡规模判断。
...
- 同步任务的恢复范围由框架、communicator 与并行布局决定；若目标实现需要整个 worker group 从 checkpoint 重启，周期失效下的期望回滚可先用半个间隔估算，再另加检测、排队、加载与质量验证。是否保留人工响应由本地事件账和结果 SLO 判断，不由 128 卡标签决定([§20.4.1](../第四部分-训练系统/第20章-稳定性与容错.md#2041-失败率模型故障为什么是常态))。
- 慢节点诊断应包含同角色计算、通信和数据等待基线以及温度/频率旁证；只有持续位于不可隐藏关键路径时，单节点劣化才按全任务损失核算([§20.4.4](../第四部分-训练系统/第20章-稳定性与容错.md#2044-慢节点比故障更难的敌人))。
...
分三层采集,口径与 [第 29 章](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md) 对齐:
