...
特定 BF16+Adam 配置可以用 $16N$ B 做早期容量示例：BF16 权重 2 B、梯度 2 B、FP32 master 权重 4 B、两个 FP32 Adam 状态 8 B。若框架不保存梯度、使用不同优化器、量化状态、压缩、ZeRO/FSDP 分片或冗余副本，结果都会变化。最可靠的输入是目标框架对冻结 state dict 的序列化字节和各 rank 分布。内容清单还有一项容易被漏掉:**dataloader 状态**(数据流位置、shuffle RNG)必须与模型状态在同一个一致性点保存,否则续训会重复消费或漏样本——机制与校验见 [§13.4.6](第13章-训练数据管道.md#1346-续训一致性dataloader-状态也是-checkpoint-的一部分),它的字节量可以忽略,一致性语义不能。
...
本节的写入窗口和第 20 章的失败率模型可以联立出一个可代入的初筛公式——**最优 checkpoint 间隔** $\tau^* \approx \sqrt{2CM}$(Young/Daly,推导与适用条件见 [§20.4.2](../第四部分-训练系统/第20章-稳定性与容错.md#2042-有效训练时间占比稳定性的唯一北极星指标)):其中 $C$ 取训练**真正不能前进**的暂停时间——同步保存取 $\approx T_{durable}$ 、异步保存取 $T_{pause}$ ,不是总上传时间;$M$ 取会导致该任务回滚的故障间隔([§20.4.1](../第四部分-训练系统/第20章-稳定性与容错.md#2041-失败率模型故障为什么是常态) 的失败率模型给出估法)。间隔定了,本节的突发写就有了频率,存储的突发 SLO 才能反推。公式已登记附录 B(F22);它是初筛,相关故障、多级快照与检测延迟下的修正见第 20 章。
...
checkpoint 不是只写不读的保险单,三类读负载的画像完全不同,存储方案必须分别对账:

- **续训加载**:故障恢复或计划重启时,全体 rank 并行拉取各自分片——瞬时读带宽需求与 14.4.3 的突发写同量级,且发生在恢复关键路径上,直接进第 20 章的恢复时间预算(MTTR 里常被低估的一段就是"拉 checkpoint")。要单独对账的是**拓扑变化的重分片读**:续训时并行度或 ZeRO/FSDP 切分方式改变,每个 rank 需要读取的字节不再等于自己写下的分片,最坏情况接近全量读放大,恢复时间按新切分实算,不能沿用"写多快就读多快"。
- **评测加载**:频率高、单次规模小(常只取权重、不取优化器状态),且多个评测任务可能并发拉同一个 checkpoint——这是 14.4.2 缓存加速层最对口的读场景;把评测读打到主存储后端上,会和训练的突发写在同一条 SLO 上排队。
- **导出转换**:训练格式到推理格式的转换(合并分片、剥离优化器状态、精度转换、序列化为推理侧格式)是一条"全量读 + 全量写"的批处理,还伴随合并分片时的 CPU 与主机内存峰值(单机合并一个大模型的 state dict 常在内存上翻车)。它不在训练关键路径上,但在**上线关键路径**上——模型发布的流转与卡点在第 30 章。

三类读加上一类写,才是 checkpoint 存储的完整流量画像;只按写路径选型,会在第一次大规模故障恢复时补课。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| RPO / RTO | 可容忍的数据(训练进度)丢失量 / 可容忍的恢复耗时;checkpoint 策略与存储分层的业务约束输入 | §14.4 |
| 三层存储 | 容量与权威副本层、共享高吞吐层、节点/机架内暂存层的职责分工;技术选型按属性验收而非产品清单 | §14.4.1 |
| staging(暂存) | 保存前先把训练态复制到 CPU 内存或本地盘、再后台持久化的中转步骤;缩短训练暂停但最新版本未必耐故障 | §14.2、§14.4.3 |
| DCP(PyTorch Distributed Checkpoint) | PyTorch 的分布式保存接口:各 rank 协同写分片,async_save 把 D2H 复制与训练重叠 | §14.2 |
| durable(可恢复)完成时间 | 排队 + 准备 + 最慢 rank 的写入 + 提交 + 校验的总和;"写 API 返回"不等于"版本可恢复" | §14.4.3 |
| commit marker / 原子提交 | 全部分片确认成功后写入的提交标记(或 latest 指针原子切换);读到未提交版本等于读到废墟 | §14.4.3 |
| 垃圾回收安全窗 | 新版本通过提交与保留条件之前不删旧版本的保护期;提前删除等于自断退路 | §14.4.3 |
| Young/Daly 公式 | checkpoint 间隔的一阶初筛:W ≈ √(2μC)(μ 为平均故障间隔,C 为保存成本);假设周期保存与 Poisson 故障 | §14.4.3 |
| 分片保存 | 各 rank 直接写自己持有的状态分片,避免聚合到单 writer;换并行度恢复时需要 reshard | §14.5.1 |
| 增量/差分 checkpoint | 只保存相对基线的变化;收益取决于真实变化率,代价是恢复链、校验与 GC 复杂度 | §14.5.1 |
| 同相突发 | 多个任务按整点等固定周期同时触发保存,叠加后打爆共享存储;平均带宽掩盖不了窗口内拥塞 | §14.3、§14.4.3 |
| 恢复演练(restore drill) | 定期在隔离作业中真正加载 checkpoint 验证可恢复性;"文件存在"不是"能恢复"的证据 | §14.4.3 |
