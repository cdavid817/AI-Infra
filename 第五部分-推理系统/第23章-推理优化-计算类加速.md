...
**下面先拆单步耗时，再逐项评估计算类加速手段。**

### 23.0.3 单步耗时的构成:先归因,再优化
...
常见的简化推导把 `v` 隐含固定为 1，再据此给出通用接受率阈值——这在生产上不成立：`v` 会随 batch、序列长度、树/链结构、kernel、图命中和并发改变；盈亏点必须使用目标负载实测的 `c`、`v` 与接受长度分布求解。
...
图 23-5:此图用于枚举叠加时的依赖边；颜色只是分类示意，不能替代上表中的目标版本实测——量化、融合与图模式之间同样可能冲突。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| Amdahl 上界 | 局部加速对整体的收益上限:1 / ((1−f) + f/s),f 为该段占比、s 为局部加速比;占比不大,加速再猛也没用 | §23.0.3 |
| 互斥区间 | 把墙钟关键路径拆成 CPU/调度 gap、设备 kernel、通信、采样后处理等不重叠区段再归因;来源可叠加、耗时不能直接相加 | §23.0.3 |
| weight-only 量化(W4A16 等) | 只压权重存储、计算前反量化回高精度;省显存与权重搬运带宽,不省算力,利好 memory-bound 的 Decode | §23.1 |
| PTQ / QAT | 训练后量化(用校准集统计缩放)与量化感知训练(训练中模拟量化);质量-成本谱系的两端 | §23.1 |
| GPTQ / AWQ / SmoothQuant | 三种主流量化算法:逐层误差补偿 / 按激活感知保护关键权重 / 把激活的量化难度平移到权重侧 | §23.1 |
| 校准集(calibration set) | PTQ 用来统计数值分布的样本集;要覆盖线上语言、模板与长度分布,且不得与验收集重合 | §23.1 |
| 双门禁 | 量化上线的次序纪律:先过质量差异门禁,再在同调度同 trace 下过性能门禁 | §23.1 |
| 算子融合(垂直/水平) | 合并相邻算子避免中间张量往返 HBM / 合并独立小算子凑满 kernel;收益取决于被删除的流量与 gap 是否真在关键路径 | §23.2 |
| FlashAttention | IO 感知的精确注意力算法:分块计算 + online softmax,不物化完整得分矩阵,大幅减少 HBM 读写 | §23.2 |
| 图模式 / shape bucket | 复用稳定的 GPU 提交结构消除 launch 开销;动态负载按形状分桶捕获,桶数在命中率与显存/冷启动之间权衡 | §23.3 |
| graph hit / fallback | 图真正被重放 vs 回退到逐算子路径的比例;验收图模式先看这两个数,再看端到端 | §23.3 |
| 投机解码(speculative decoding) | 小草稿模型先猜多个 token、目标模型一次并行验证;正确采样协议下保持目标分布,收益由接受率与草稿/验证成本共同决定 | §23.4 |
| 接受率 α / 草稿成本 c / 验证成本 v | 投机盈亏三参数:E(α,k)/(k·c+v) > 1 才赚;v 随 batch、树结构与图命中变化,必须实测 | §23.4 |
| Pareto(质量-吞吐-延迟) | 通过质量与成功率门禁的候选在多目标平面上的非支配前沿;不预设"保守/激进"赢家 | §23.1、§23.6 |
