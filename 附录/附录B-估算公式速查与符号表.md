...
| N_eff | 训练算力估算的有效参数量(稠密 N_eff = N;MoE 含激活专家、共享层与 router 的实际计算) | 个 | [§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) |
...
| M_x | 某类显存占用(下标注明对象) | Byte | [§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心)、[§17.3](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#173-原理与量化模型) |
...
| R | Offload 搬运/计算时间比 | 无量纲 | [§17.3.4](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#1734-offload救命稻草与自欺欺人的分界线) |
...
| E | 运行能耗(SCI 口径) | kWh(细粒度用 J) | [第 31 章「从电费到碳账」](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3134-从电费到碳账功能单位与系统边界) |
| I | 电网碳强度(SCI 口径,随地区与时段变化) | gCO2e/kWh | [第 31 章「从电费到碳账」](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3134-从电费到碳账功能单位与系统边界) |
| M | 硬件隐含排放(SCI 口径,制造与运输碳按折旧分摊) | gCO2e | [第 31 章「从电费到碳账」](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3134-从电费到碳账功能单位与系统边界) |
| R | 功能单位(SCI 口径:请求 / token / 训练任务 / Agent 任务完成) | 按负载声明 | [第 31 章「从电费到碳账」](../第七部分-工程化与治理/第31章-容量、成本与SLO.md#3134-从电费到碳账功能单位与系统边界) |
...
| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F1 | C = 6 · N_eff · D | 参数量 → 训练总算力 | 稠密 N_eff = N;MoE 以激活专家、共享层和 router 的实际计算为基础,不能直接用总参数;长上下文补注意力项 ≈ 每 token 12·L·s·h;重计算把系数 6 推向 7~8 |
| F2 | T = C ÷ (n_卡 · P_峰值 · MFU) | 训练工期 | MFU 是唯一必须实测的量;稠密大规模训练合理区间 35%~45%,MoE 与长上下文更低 |
| F3 | D ≈ 20N(Chinchilla 假设下的参照线);过训练 D/N = 100~500 | 审视需求合理性 | 参照线不是普遍门槛;偏离 20N 时应记录所依据的 scaling 假设与推理成本策略([§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) 公式六) |

### B.2.2 显存占用(第 6、17 章)

| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F4 | M_训练静态 = (2 + 2 + 12) · N = 16N Byte | BF16 混合精度 + Adam 的模型状态(参数 + 梯度 + FP32 master/动量/方差) | ZeRO-1/2/3 分别把 12N、+2N、+2N 项除以 DP([§17.3.1](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#1731-显存账本先算清楚再谈优化)) |
...
| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F8 | M_KV/token = 2 · L · a_kv · d_head · p Byte | 每 token KV Cache | 系数 2 = K + V;**KV 压缩比 r = a / a_kv**(MHA r=1 基线、GQA 常见 4~8、MQA r=a、MLA 按潜维折算等效压缩可达一到两个数量级,见 §6.3);滑动窗口封顶 |
...
| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F10 | 训练吞吐(token/s)= n_卡 · P_峰值 · MFU ÷ (6 · N_eff) | F2 的另一写法 | 同 F2 |
| F11 | MFU = 实测吞吐 × 6 · N_eff ÷ (n_卡 · P_峰值) | 从实测反算 MFU | Decode 类 memory-bound 负载不用 MFU 考核,用带宽利用率([§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心)) |
...
| F20 | T_comm ≈ V_通信量 ÷ B_eff + 启动延迟项 | 通信耗时 | — | B_eff 按跨过的最低层级取值,以同消息大小、同拓扑的基准实测回填,不能固定按标称带宽打折;同一公式换 B_eff 结论差一个量级 |
...
### B.2.8 推理容量与缓存命中(第 24、31 章)

| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F23 | T_prefill_eff = QPS × L_in × (1 − h) | 前缀缓存命中率对 prefill 侧有效容量的折算 | h 为命中 token 占输入 token 比例(h = h_max × P_路由 × P_驻留,§24.4.3);h 取线上实测并做跌落敏感性分析,不取理论上限(§31.1.1);请求画像用 (L_in, L_out, h) 三元组 |

### B.2.7 Checkpoint 间隔(第 14、20 章)

| # | 公式 | 用途 | 关键提醒 |
|---|---|---|---|
| F22 | τ* ≈ √(2CM) | 最优 checkpoint 间隔初筛(Young/Daly) | C 取训练不能前进的暂停:同步保存 ≈ T_durable、异步保存取 T_pause(§14.4.3),不是总上传时间;M 取会导致回滚的故障间隔(§20.4.1);假设周期保存 + Poisson 故障,相关故障、多级快照与检测延迟下需完整模型(§20.4.2) |
...
| FP16 | 2 | ±65504(5 位指数) | ~3 位 | Volta 起;各家普遍支持 | 推理;训练需 loss scaling([§17.3.5](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#1735-混合精度机制master-weights-与-scaling-的层次)) |
...
| FP8 E5M2 | 1 | ±57344 | <1 位 | 同上 | 梯度(要范围不要精度);scaling 策略见 [§17.3.6](../第四部分-训练系统/第17章-显存与训练侧精度机制.md#1736-亚-8-比特训练的工程门槛fp8-与更低) |
