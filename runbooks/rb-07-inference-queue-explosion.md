...
- 引擎 waiting 队列长度持续攀升不回落;排队时长占 TTFT 的比例从常态的个位数百分比升到 50% 以上(排队时长与 TTFT 的定义与采集点见 [§29.2.1](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md#2921-llm-专属指标体系定义采集点与误用),调度器 waiting/running/token 预算三件套见 [§22.2](../第五部分-推理系统/第22章-推理引擎原理.md#222-批处理与调度))。
...
3. **若确认存在长 prompt 队头阻塞,临时下调单请求最大 prompt 长度上限**(机制见 [§22.2.2](../第五部分-推理系统/第22章-推理引擎原理.md#2222-chunked-prefill长-prompt-饿死-decode))。副作用:超长请求被直接拒绝,长文档类业务功能受损,需要通知对应业务方。
...
- waiting 队列长度、running 批次大小、每步 token 预算利用率、抢占率、KV 池水位(引擎指标,采集点见 [§22.2](../第五部分-推理系统/第22章-推理引擎原理.md#222-批处理与调度) 与 [§29.2.1](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md#2921-llm-专属指标体系定义采集点与误用))。
...
判定依据说明:三个分支用三组互相独立的指标区分——流量突增看网关入口计数,容量塌缩看健康实例数,长 prompt 阻塞看请求画像加 TPOT 周期毛刺(长 Prefill 阻塞 Decode 的特征,见 [§22.2.2](../第五部分-推理系统/第22章-推理引擎原理.md#2222-chunked-prefill长-prompt-饿死-decode))。先查前两个,因为它们的证据是纯计数、最不易误判。
...
- **分支四(准入参数错配)**:回滚最近的引擎调度配置变更(token 预算、最大并发、KV 池预留比例),按 [§22.2.4](../第五部分-推理系统/第22章-推理引擎原理.md#2224-把本章的账合起来算一次) 的算账方法重新核定参数。
...
- **监控**:排队时长、waiting 长度、健康实例数三者分别设告警,不要只报端到端 TTFT——三个分支各有直达信号([§22.2](../第五部分-推理系统/第22章-推理引擎原理.md#222-批处理与调度)、[§29.2.1](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md#2921-llm-专属指标体系定义采集点与误用))。
