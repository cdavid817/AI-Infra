...
**边界声明:不写边界的碳数字没有可比性。** 同一个训练任务,算不算数据管道(第 13 章的清洗与预处理消耗的机器)、算不算为它闲置预留的卡、散热与配电按 PUE 分摊还是只计 IT 设备、checkpoint 存储计不计——每一项取舍都能让结果差出数倍。所以报告碳数字的最低纪律与报告 MFU 相同([§29.2.1](../第六部分-上层平台/第29章-可观测性与评测流水线.md#2921-llm-专属指标体系定义采集点与误用) 的口径纪律):数字旁边写明系统边界与功能单位,读者才能判断两个数字是否在比同一件事。
