...
站在两条主线的终点回望全程，训练链路的每一段耗时、推理链路的每一毫秒延迟，最终都要折算成三个数字：需要多少卡(容量)、花多少钱(成本)、对外承诺什么(SLO)。这是全书的收口章。

这本总账沿用三组前文口径：[§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) 的参数量与资源换算公式，[§12.5](../第二部分-算力底座/第12章-国产算力平台与异构迁移.md#125-性能对标的正确姿势全书唯一定义处) 的性能对标口径与 [§12.7](../第二部分-算力底座/第12章-国产算力平台与异构迁移.md#127-生态作为选型变量全书唯一定义处) 的生态人月折算方法，以及 [§4.1.4](../第一部分-基础与心智模型/第04章-硬件第一性原理、架构范式与数值精度.md#414-规模层级互联域是关键抽象) 和第 7 章定义的互联域概念。[§4.1.5](../第一部分-基础与心智模型/第04章-硬件第一性原理、架构范式与数值精度.md#415-功耗与散热一句伏笔) 留下的功耗与散热约束，在 [§31.2](#312-机房功率与散热全书唯一定义处) 完整展开。
...
这一步还必须接上 [§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) 的三态 token 口径,补两个一等参数。其一,**输入/输出成本不对称**:prefill/decode 分开算只是第一层,报价与方案对比时还要记住两侧的单位机时成本差约一个数量级——输出 token 主导的负载(长回答、推理链)与输入主导的负载(长文档、RAG)在同样"总 token 量"下的卡数需求完全不同。其二,**前缀缓存命中率 $h$** 必须显式进模型:命中的输入 token 不消耗 prefill 算力,

$$T_{prefill}^{eff} = QPS_{peak} \times L_{in} \times (1 - h)$$

$h$ 从 0 到 0.8,prefill 侧算力需求直接砍掉八成——在 PD 分离部署下这就是 prefill 池的卡数乘数,也是 **Agent 时代容量模型里波动最大的变量**(多轮与 Agent 负载的 $h_{\max}$ 天然高,但一次路由变更或灰度切流就能把实际 $h$ 瞬间打低,见第 24 章)。两条纪律:$h$ 取线上实测分布(数据来源是第 29 章的成本可观测性,按租户/模型分列),不取理论上限;容量必须对"$h$ 跌到保守值"做敏感性分析——按峰值 $h$ 规划的容量,会在下一次网关变更时被 prefill 突增击穿。请求画像至此完整:**(输入 token 数, 输出 token 数, 前缀命中率)** 三元组,与第 27 章计费口径同源。
...
大数据时代的采购单元是"台",一台 x86 服务器十几万,容量曲线几乎连续。超节点([§4.1.4](../第一部分-基础与心智模型/第04章-硬件第一性原理、架构范式与数值精度.md#414-规模层级互联域是关键抽象)、第 7 章)把最小采购单元变成了"域"或"整柜":域内高带宽互联是预制的,不能只买半柜。这带来三个后果:**第一**,容量台阶从几十万一级跳到千万一级,"按需小步扩容"策略失效,规划必须一次看 12–24 个月;**第二**,超配成为常态——需求算出来 1.3 个域,你只能买 2 个,多出的 35% 容量要么找业务填满、要么摊进成本;**第三**,故障域同步变粗(第 20 章),冗余也要按域规划,N+1 里的"1"从一台变成一柜。对做平台的人意味着:容量规划文档里必须显式写出"取整损耗",并把它作为与业务方谈资源承诺的筹码,而不是默默吞掉。
...
[§4.1.5](../第一部分-基础与心智模型/第04章-硬件第一性原理、架构范式与数值精度.md#415-功耗与散热一句伏笔) 只埋了一句话,这里给出完整定义:**功率与散热约束,指数据中心的供电容量、机柜功率密度上限和散热方案,共同决定了(a)一个机柜物理上能部署多少卡、(b)已部署的卡能否长时间跑在标称算力上。** 后续任何章节谈及功耗、散热、机柜密度,均以本节为准。
...
![三种散热方案在机柜功率密度轴上的适配区间:风冷 ≤20 kW/柜,冷板式液冷 20–130 kW/柜,浸没式液冷 ≥100 kW/柜,越界强行沿用则热节流](../diagrams/generated/ch31-cooling-bands.svg)

<!-- source: diagrams/sources/ch31-cooling-bands.excalidraw -->
...
![年化 TCO 五块构成的典型占比区间:硬件折旧 55–65%、机房改造摊销 8–12%、电费 10–18%、运维人力 8–15%、生态折算 0–15%](../diagrams/generated/ch31-tco-composition.svg)

<!-- source: diagrams/sources/ch31-tco-composition.excalidraw -->

图 31-3:年化 TCO 的五块构成。生态折算是五项里最常被漏记的一项——漏掉它的 TCO 对比,结论可能整个反转。

**五块之外的补充清单:安全与合规成本。** 可信平面与模型供应链(第 2、30 章)的机制不改变上面的公式,但会在三处加数,预算时按清单核对:其一,**签名、准入与审计基础设施**——签名服务与透明日志的接入、注册表准入卡点、审计日志的存储与查询,建设是一次性工时,长期运维进 $C_{ops}$;其二,**机密计算的双重溢价**(若威胁模型需要,边界见第 30 章)——支持 TEE 的硬件与配套证明服务抬高 $C_{hw}$,性能折损则作用在有效吞吐上,同样的 SLO 需要更多实例,折价幅度应按目标硬件实测而非取厂商宣传值;其三,**合规审计人力**——删除传播的执行、数据集许可证审查(第 13 章)、对外交付物料清单与审计证据,是随监管与客户要求刚性增长的人头,进 $C_{ops}$ 。这三项在多数团队的第一版 TCO 里是零,不是因为不存在,而是因为还没被要求;一旦模型对外交付或数据跨组织流动,它们会以合同条款的形式变成硬成本——事前入表,比事后追加体面得多。

### 31.3.2 国产卡在 TCO 模型里的正确算法

国产卡进 TCO 模型,常见两种错误算法:一种只比卡价,得出"便宜 40%"的结论;一种只比单卡跑分,得出"性能差一截"的结论。两种都不成立,正确算法分三步:

**第一步,分子用有效吞吐,口径按 [§12.5](../第二部分-算力底座/第12章-国产算力平台与异构迁移.md#125-性能对标的正确姿势全书唯一定义处)。** 不用单卡峰值跑分,直接测目标负载在质量与 SLO 门禁下的端到端集群产出；MFU 与规模效率用于解释差距,不再重复相乘生成吞吐。目标平台的软件栈、并行与通信影响必须通过目标规模和可持续算力实测体现([§31.2.3](#3123-降频与热节流性能不稳定的头号真凶))。

<!-- claim: CLM-031-014; classification: estimate -->
**第二步,加上生态人月,方法按 [§12.7](../第二部分-算力底座/第12章-国产算力平台与异构迁移.md#127-生态作为选型变量全书唯一定义处)。** 算子补齐、精度对齐、性能调优、双后端 CI 的工时分成一次性与年度持续投入,由缺口原型、历史交付与情景区间估算,再乘人力全成本进入 $C_{eco}$ 。它可能高于、低于或接近硬件价差,不能用模型规模映射成通用人月区间。
...
**成本归因**统一用一个指标:**每百万 token 成本**(推理)或**每万亿训练 token 成本**(训练),即年化 TCO 除以年实际产出 token。它把利用率、MFU、SLO 折扣全部吸收进一个数,可横向比较自建/云/不同硬件。**优化杠杆排序**按"投入产出比"从高到低:①利用率——从 30% 拉到 60% 等于成本减半,零硬件投入,大多数集群的最大红利在这里(第 11 章);其中**离线负载填谷**是见效最快的一招:离线批量推理与评测这类可抢占、无 SLO 的负载([§26.4.4](../第五部分-推理系统/第26章-特殊负载.md#2644-离线批量推理吞吐-only与在线服务处处相反))填进推理潮汐低谷与训练空窗([§9.6](../第二部分-算力底座/第09章-批调度器.md#96-训练与推理混部潮汐利用及其边界)),同一批卡多产出一份收入侧 token,分母直接变大;②推理引擎层优化——量化、batch 策略、PD 分离与前缀缓存命中率(命中率对 prefill 池的杠杆见 31.1.1,常大于量化),软件工时换卡数;③采购与部署结构——水位、粒度、混合云错峰;④散热与电费——PUE 优化,量级 10% 上下;⑤硬件换代——杠杆最大但资本开支也最大,放最后是因为前四项没做完之前,换代只是把浪费搬到更贵的卡上。

### 31.3.4 从电费到碳账:功能单位与系统边界

[§31.2.4](#3124-pue-与电费功率如何进入-tco) 把功率折成了电费,本节把同一度电再折一次:折成碳。做这件事不需要新的测量设施,需要的是一套口径——让"这个集群、这个服务排了多少碳"变成一句可比较、可复核的话,而不是一个孤立数字。

<!-- claim: CLM-031-017 -->
业界通行的口径框架是软件碳强度(SCI)的"每功能单位碳排"(C per R):碳排 C 由两部分构成——**运行排放**,即能耗 E 乘以所用电网的碳强度 I(单位 gCO2e/kWh,随地区与时段变化);**隐含排放** M,即硬件制造与运输阶段的碳按折旧思路分摊到本系统、本时段的份额。两部分加总后除以功能单位 R,且所有要素必须按同一个 R 缩放——分子分母口径错位的碳数字没有意义。框架落到工程上只剩两个选择:R 选什么,边界画到哪。

**功能单位:三类负载各选各的 R**,而且分母与 [§31.3.3](#3133-全书技术决策的成本折算) 的成本归因指标可以直接共用:

| 负载 | 功能单位(R)候选 | 对应的成本指标 |
|---|---|---|
| 训练 | kWh 或 gCO2e / 一次训练任务;gCO2e / 万亿训练 token | 每万亿训练 token 成本 |
| 推理 | Wh / 请求;J / token;gCO2e / 百万 token | 每百万 token 成本 |
| Agent | 能耗或 gCO2e / 一次任务完成(含模型调用、工具执行与沙箱开销,第 28 章) | 每任务完成成本 |

**边界声明:不写边界的碳数字没有可比性。** 同一个训练任务,算不算数据管道(第 13 章的清洗与预处理消耗的机器)、算不算为它闲置预留的卡、散热与配电按 PUE 分摊还是只计 IT 设备、checkpoint 存储计不计——每一项取舍都能让结果差出数倍。所以报告碳数字的最低纪律与报告 MFU 相同([§29.2.1](../第七部分-工程化与治理/第29章-可观测性与评测流水线.md#2921-llm-专属指标体系定义采集点与误用) 的口径纪律):数字旁边写明系统边界与功能单位,读者才能判断两个数字是否在比同一件事。

**测量与估算是两个档次,必须标注。** 测量口径 = PDU/机柜电表(或 IPMI 读数)的实际用电量,除以网关侧的请求与 token 计数;估算口径 = TDP × 估计利用率 × 时长的近似。两者的差来自三处:空闲功耗(不跑任务的卡也在耗电,估算最容易漏)、散热与配电分摊(PUE 是年均值,不随负载线性)、功率随负载波动(TDP 是上限不是均值)。本书配套 TCO 计算器的 `carbon_intensity` 可选输入走的就是估算口径,其输出已明确标注"估算口径、非审计"——两种口径都可用,混用而不标注才是问题。本章不给任何 J/token 或电网碳强度参考值:前者请用上述测量口径在自己的负载上实测,后者查所在电网的公开碳强度数据(多数电网运营商与公共数据库按地区、分时段发布)。

**抵消不降分。** SCI 的一个明确设计决定是:中和、抵消类手段(购碳汇、绿证)不降低分数。这意味着环境指标只能靠直接效率改进拉动——MFU、利用率、每 token 能耗。[§31.3.3](#3133-全书技术决策的成本折算) 说利用率是最大的成本杠杆,这里补全后半句:**它同时是最大的碳杠杆**——闲置的卡照常耗电、照常分摊隐含碳,利用率翻倍则每 token 的成本与碳排同时近乎减半,同一个优化两本账同时受益。这也是碳指标应该挂进第 11 章既有治理体系、而不必另建一套考核的理由。

<!-- claim: CLM-031-019 -->
**两类"降碳手段"必须按每功能单位总账核算,而非只看功率曲线。** 其一,power cap:压低的是瞬时峰值功率,但持续算力同步下降、训练工期拉长,总能耗与总排放未必下降——卡在低效工作点上多跑的天数可能吃掉削峰省下的部分,判据只有一个:每功能单位总排放是否下降。其二,碳感知调度:把批任务迁到电网碳强度低的时段或地区,对训练类可排队负载在机制上成立,且与峰谷电价错峰完全同构、同一套调度能力可以两用;但它的前提是数据与 checkpoint 可低成本随行——第 14 章的数据分发与 checkpoint 体量、第 9 章的批调度能力共同决定这个前提是否成立,数据重力大的任务迁移开销可能反超碳收益;在线推理被 SLO 与就近接入锚定,基本不可迁,可做的只剩本地时段维度的错峰填谷。

<!-- claim: CLM-031-018 -->
最后一句定位:SCI 的 AI 扩展规范(SCI for AI)把上述框架细化到 AI 生命周期各阶段(数据准备、训练、部署、推理),按系统类型定义边界与功能单位;其细化程度属高时效信息,以 `references/claims/chapter-31.yaml` 对应条目登记的来源为准。能源相关符号(E、I、M、R)已并入附录 B 符号表,本节为其定义处。
...
本章数字密度为全书之最,按四类口径区分,逐条登记于 `references/claims/chapter-31.yaml`(`CLM-031-001` 至 `CLM-031-019`),正文以 `<!-- claim: ... -->` 标记锚定:

- **合成案例**:[§31.0.1](#3101-问题场景买够了卡机房上不了电) 的全部人物、机构与数字(1,200 QPS、512+256 卡、132 kW 柜、1,900 万年增支出等)为写作构造的示意(`CLM-031-001`),不构成真实事故记录。
- **估算模型,非报价**:训练卡数 2.5–4 倍折扣、电费算例(约 900 万/720 万)、生态人月折算方法、优化杠杆量级(MFU +5pp ≈ 卡数 −10% 等)均为公式推导的估算示例,输入、公式、假设与误差来源见对应 claim 的 `estimate` 字段;TCO 相关数字用于说明方法与校验漏项,不能替代按自身电价、卡价与人力成本的实际测算,更不是任何厂商报价。
...
- **碳口径**:碳相关内容(SCI 框架、SCI for AI 与调度手段边界,`CLM-031-017` 至 `CLM-031-019`)只给功能单位、系统边界与测量方法,不给 J/token 或电网碳强度数值——前者应自测,后者应查所在电网的公开数据;TCO 计算器的碳排输出为估算口径、非审计数据。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| goodput 饱和点 | P99 延迟越过 SLO 的吞吐位置,永远先于"吞吐不再增长"的硬饱和点到来;容量模型只用前者 | §31.1.3 |
| 容量水位 ρ | 日常负载不超过 goodput 饱和点的比例(推理典型 0.6–0.75);取值看峰谷比、扩容速度与故障域大小 | §31.1.3 |
| 整机开销系数 α | 从卡功耗折算整机功耗的放大系数(CPU、内存、风扇、互联,典型 1.3–1.5) | §31.1.1 |
| PUE(Power Usage Effectiveness) | 数据中心总耗电与 IT 设备耗电之比,散热与配电开销的放大系数;电费公式的必乘项 | §31.2.4 |
| 功率密度 | 单柜功耗:大数据机房按 5–10 kW 设计,超节点整柜 100–150 kW——配电、散热、承重、PUE 全由它派生 | §31.2.1 |
| 风冷 / 冷板式液冷 / 浸没式液冷 | 三档散热方案,适配密度区间分别约 ≤20、20–130、≥100 kW/柜;方案是密度的函数,不是技术偏好 | §31.2.2 |
| 热节流(thermal throttling) | 结温逼近上限时固件自动降频保护硅片;不报错不告警,唯一表征是"变慢"——性能不稳定的头号真凶 | §31.2.3 |
| 可持续算力(sustained throughput) | 满载 1 小时以上的稳态吞吐,区别于前 10 分钟峰值;到货验收与对标的必测口径 | §31.2.3 |
| TCO 五块 | 年化总账:硬件折旧、机房改造摊销、电费、运维人力、生态折算;占比区间用于校验漏项而非照抄 | §31.3.1 |
| 每百万 token 成本 | 年化 TCO 除以年实际产出 token 的统一归因指标,可横比自建/云/不同硬件 | §31.3.3 |
| SCI(软件碳强度) | 每功能单位碳排框架:运行排放(能耗×电网碳强度)+ 隐含排放(制造碳分摊),除以功能单位 R;抵消不降分 | §31.3.4 |
| 功能单位 R 与系统边界 | 碳与能耗数字的分母(每请求/每 token/每任务)与计入范围声明;不写边界的碳数字没有可比性 | §31.3.4 |
| 隐含排放 M | 硬件制造与运输阶段的碳按折旧思路分摊到本系统本时段的份额 | §31.3.4 |
| power cap | 压瞬时峰值功率的手段;持续算力同降、工期拉长,每功能单位总排放未必下降——按总账核算 | §31.3.4 |
| 碳感知调度 | 把可排队批任务迁到电网碳强度低的时段/地区;与峰谷电价错峰同构,前提是数据与 checkpoint 可低成本随行 | §31.3.4 |
| 峰谷比 | 峰值与谷值负载之比;决定自建利用率下限、混合架构收益与容量水位取值 | §31.4 |
