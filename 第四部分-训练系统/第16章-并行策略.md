...
<ChapterContext
  ...
  upstream="第 2 章主线 A 的稳态 step 结构；第 6 章形状与资源账；第 7 章互联域；第 14 章分片状态保存"
...
[§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) 提供参数、激活与 FLOPs 的入口，[§7.4.2](../第一部分-基础与心智模型/第07章-互联与集合通信.md#742-集合通信原语四个动词与它们的通信量) 提供 collective 的通信量模型，[§14.4](../第三部分-数据底座/第14章-存储与Checkpoint-IO.md#144-原理与量化模型) 说明分片网格如何进入 checkpoint，[§17.3](./第17章-显存与训练侧精度机制.md#173-原理与量化模型) 处理状态分片、重计算与 offload。
...
第 6 条值得单独展开,因为它是续训改配置时最先撞上的墙:**并行配置与 checkpoint 格式是耦合的**。每个并行网格都把训练状态切成了特定分布——TP 分片的权重、ZeRO/FSDP 切分的优化器状态、EP 的专家归属、PP 的层归属;改动任何一维,旧 checkpoint 都不能按原样加载。出路两条:依赖框架的分布式 checkpoint 重映射(加载时按新网格重切,伴随 [§14.4.4](../第三部分-数据底座/第14章-存储与Checkpoint-IO.md#1444-读路径三类加载的画像) 的重分片读放大),或先离线转换为与并行布局无关的规范格式再重切。并行方案评审必须把"该网格的 ckpt 能否转到候选网格、转换工具是否存在且验证过"列为显式检查项——这道墙比第 20 章讨论的"变卡数续训的结果漂移"更前置:格式转不过去,连产生漂移的机会都没有。
...
显存收益也不能只写“除以并行度”。权重、梯度、optimizer state、保存激活、临时聚合 buffer 分栏计算；某一轴只缩减其实际分片的对象。比如 EP 缩减本 rank 持有的专家参数，但共享参数仍按其 DP 组同步；CP 可缩减部分序列激活，却不自动分片权重。计算方法接 [§17.3.1](./第17章-显存与训练侧精度机制.md#1731-显存账本先算清楚再谈优化)。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| DP(数据并行) | 每副本持完整模型、各算不同数据,每步以梯度同步收尾;切的是 batch | §16.4.1 |
| TP(张量并行) | 把层内的权重矩阵切到多卡、每层前反向都要集合通信;高频通信,必须收在高速域内 | §16.4.1 |
| PP(流水线并行) | 按层切成 stage、微批依次流过;通信量小,代价是灌入排空的气泡 | §16.4.1 |
| EP(专家并行) | 把 MoE 的专家分布到多卡,token 经 All2All 分发与回收;负载由最忙 rank 决定 | §16.4.1 |
| SP / CP(序列/上下文并行) | 狭义 SP 切 TP 组内特定激活;CP 把整个序列切到多卡,attention 需跨卡交换 KV——长序列训练的解法 | §16.4.1 |
| 并行网格(parallel mesh) | (t,p,d,e,c,m,b) 的组合与 rank 到物理卡的映射;总卡数关系须服从目标框架的 mesh 定义 | §16.4.4 |
| DeviceMesh / DTensor | PyTorch 表达 N 维设备布局与分片张量语义的原生抽象 | §16.2、§16.4.7 |
| microbatch | 流水线内一次推进的小批;数量 m 越大,理想气泡率 (p−1)/(m+p−1) 越低 | §16.4.3 |
| GPipe / 1F1B / interleaved | 三代流水线调度:全前向再全反向 / 一前向一反向交错(压激活驻留)/ 虚拟 stage 交错(再降气泡、多付 P2P) | §16.4.3 |
| 气泡率(bubble) | 流水线灌入排空造成的空闲占比;实测还要叠加 stage 失衡与通信暴露 | §16.4.3 |
| 可行域 | 并行候选依次过显存、整除、批量语义、拓扑四道机械门禁,再进短跑实测——不从模型规模直接查表 | §16.4.4 |
| collective trace | 目标框架实际产生的集合通信事件清单(类型、次数、字节);通信账以它为准,不用固定口诀 | §16.4.2 |
| padding / packing | 变长序列补齐到定长造成的浪费 / 多条短序列拼进一条定长的对策;packing 须验证 mask 与 loss 归一化语义 | §16.4.6 |
| makespan | 一批并行工作的完成时间,由最忙/最慢的参与者决定;MoE 的 EP 负载核算用它而非平均值 | §16.4.6 |
| 短跑(short run) | 冻结其余变量、小规模短时对照实验;并行选型的性能结论只能由它给出,容量模型只负责缩小空间 | §16.4.8 |
