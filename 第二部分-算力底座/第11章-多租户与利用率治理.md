...
$$0.85 \times 0.62 \times 0.40 = 0.2108$$
...
**推理侧的对等口径【与训练 MFU 并列的单点定义】。** MFU 只对 compute-bound 阶段有意义。decode 阶段是 memory-bound([§6.1](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#61-算子与瓶颈归属先知道钱花在哪)):即便引擎已经打满带宽,MFU 也天然停在个位数到十几个百分点——拿 MFU 考核推理集群,会得出"永远不达标"的伪结论,或诱导用加大 batch 牺牲延迟去刷一个不该刷的数。推理侧的利用率必须换四个指标:

| 指标 | 定义 | 回答什么 |
|---|---|---|
| **带宽利用率(MBU)** | 每 token 实际搬运字节数(权重 + 当前批 KV)× token 吞吐 ÷ 峰值 HBM 带宽 | decode 阶段的"MFU 对等物":硬件带宽被模型产出用掉了多少 |
| **KV 占用率** | KV Cache 已用 ÷ KV 预算 | 双向信号:过低说明批没打满、容量闲置;持续过高预示驱逐与抖动(runbook 见 rb-08) |
| **batch 饱和度** | 实际有效 batch ÷ 当前 SLO 约束下可承载的最大 batch | 单卡吞吐被摊薄到什么程度;分母必须带 SLO 条件,不是显存上限 |
| **前缀缓存命中率** | 命中前缀的输入 token 占全部输入 token 的比例 | [§6.2](../第一部分-基础与心智模型/第06章-模型结构的定量分析.md#62-参数量与资源换算核心) 三态成本中"缓存命中"档的占比——推理成本的头号杠杆,机制与工程见第 24 章 |

两套口径的考核对象也不同:训练侧 MFU 归因到任务(并行配置、数据管道、通信);推理侧四指标归因到服务配置(副本数、批处理策略、路由与缓存策略)。第 25 章的扩缩容信号、第 29 章的服务观测、第 31 章的推理侧成本归因,统一引用本节口径,不得另造同名指标。
...
任务画像先建立共同事实：每个任务保留分配卡时、训练窗口、吞吐、MFU、数据与通信等待、失败恢复和能耗边界。闲置回收比曝光更强，必须带影子模式、通知、宽限期、白名单、申诉和一键恢复。弹性借用与超卖还要证明高优任务能在 SLO 内拿回保底，并把抢占回退、恢复失败和队列波动计入收益。回收之后的填充手段里,**用离线批量推理填谷**应排在硬塞同步训练之前:它可抢占、无延迟 SLO、没有训练状态要回滚,摩擦成本最低(负载画像与入队方式见 [§9.6](第09章-批调度器.md#96-训练与推理混部潮汐利用及其边界))。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| 分配占用率(Allocation) | 已分配卡时占可服务卡时的比例;只回答"卡是否被调度器绑定",调试容器空转也计入 | §11.3.2 |
| GPU-Util(nvidia-smi 利用率) | 采样周期内有 kernel 在执行的时间比例;等通信时照样接近 100%,不能作为产出指标 | §11.3.2 |
| SM Activity / SM Occupancy | DCGM 的活动指标:至少一个 warp 活跃的时间比例 / 驻留 warp 相对上限的比例;都是归因信号,不进核算连乘 | §11.3.2 |
| MFU_window(窗口内 MFU) | 只在有效训练 step 窗口内计算的模型算力利用率;分母必须用实际执行路径对应的稠密峰值与精度 | §11.3.2 |
| HFU(Hardware FLOPs Utilization) | 把重计算等额外硬件工作也计入分子的口径;与 MFU 定义不同,不能只凭名称换算 | §11.3.2 |
| 集群有效模型 FLOPs 占比 | 分配占用率 × 训练窗口占比 × 窗口内 MFU 的三层连乘,前提是时间边界互斥、样本一致 | §11.2、§11.3.2 |
| 生命周期空窗 | 训练 step 之外的已分配时间:镜像与 checkpoint 加载、调试空闲、失败后等待重启等 | §11.3.3 |
| 影子模式(shadow mode) | 治理动作先只记录"若执行会影响谁"而不真正执行的灰度方式,自动回收上线前的必要护栏 | §11.3.4 |
| 期望回退卡时 | 抢占一个 N 卡任务的平均损失 ≈ N × checkpoint 间隔的一半,弹性借用收益核算的扣减项 | §11.3.1 |
| 每有效 token 能耗 | 设备/PDU 功率积分除以有效产出 token;`TDP × 利用率` 既非实际功率也无完整系统边界 | §11.3.4、第 31 章 |
| 受控实验(单变量对照) | 固定其余因素、只改一个变量对照基线的归因方法;活动指标只能提假设,实验才能定责任 | §11.3.3 |
