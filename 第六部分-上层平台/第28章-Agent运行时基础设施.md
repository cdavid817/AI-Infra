...
一个数据分析 Agent 上线三周后,业务方拿着 trace 来找平台团队:端到端 P50 延迟 44 秒,其中模型推理(4 轮调用累计)只占 11 秒,真正的大头是每轮代码执行前的沙箱准备——每次执行 Python 代码,平台都新起一个容器:拉起容器 1.8 秒,安装 pandas/numpy 依赖 6.5 秒,单轮沙箱准备约 8.3 秒,4 轮下来沙箱准备合计约 33 秒,其中光依赖安装就占了 26 秒。业务方的困惑很直接:"我在本地跑同样的代码,总共不到 3 秒,为什么上了平台反而慢了十几倍?"
...
先校正一个规模直觉:**这套沙箱基础设施最大的用户不是线上 Agent 服务,而是第 21 章的 RL 训练**。线上 Agent 的沙箱并发跟着业务流量走,常态成千;agentic RL 的 rollout 按迭代脉冲打进来,峰值要求上万并发沙箱、且冷启动延迟直接进训练吞吐([§21.4.5](../第四部分-训练系统/第21章-后训练基础设施.md#2145-奖励模型与环境服务被漏算的第三块资源) 的容量算术、[§21.4.7](../第四部分-训练系统/第21章-后训练基础设施.md#2147-多轮-agentic-rl环境有了状态延迟进了关键路径) 的会话亲和)。两边在技术栈上完全同构——本节的隔离三档、冷启动优化、资源规格全部双向适用——所以执行基座应当建一套、两边共用:线上流量与训练脉冲天然错峰,合池的容量利用率高于各建一套;本章按线上视角展开,容量口径以两类负载的叠加峰值为准。
...
**第一,冷启动延迟的构成要拆开算。** 沙箱就绪时间 = 实例拉起 + 镜像/文件系统就绪 + 依赖与解释器加载。问题场景中 8.3 秒里 6.5 秒是依赖安装——这与隔离技术无关,任何一档隔离下现装依赖都是灾难。所以第一条设计规则是:**依赖必须进镜像或进预挂载的只读层,运行时零安装**。做到这一点后,容器冷启动回到 1–2 秒,微 VM 回到 200 毫秒上下,此时再谈隔离技术的选择才有意义。
...
### 28.4.4 Agent Runtime 2.0:协议、权限与持久执行
...
图上四个新组件各管一件不能混的事:Tool Registry 管注册与发现,连同每个 MCP Server 的信任等级(自建/采购/第三方);Policy Decision 在每次工具调用前做授权判定,粒度到工具级、资源级、参数级;Credential Broker 集中保管凭据,只在调用真正发出的边界注入;Durable State 把任务执行做成步级 checkpoint 加事件日志——28.4.5 的会话状态回答"对话历史在哪",Durable State 回答"任务执行到第几步、每步做了什么",长任务的跨实例恢复与事后审计回放都靠它。
...
这条边界要再钉深一层,因为它是 Agent 平台最容易出错的地方:**会话状态与 KV 缓存之间,source of truth 永远是应用层的会话状态**。KV 只是它的衍生加速视图——同一份会话历史,任何时刻都可以从文本状态重新 prefill 出语义等价的 KV;反方向不成立,也绝不允许(从 KV 反推会话内容,既做不到也不该做)。两者不一致时的裁决规则因此只有一条:**以会话状态为准,KV 作废重建**。三个必然推论:其一,会话的任何编辑(重试失败的工具调用、压缩历史、撤回消息)必须发生在应用层,然后**接受**引擎侧 KV 从编辑点起全部失效([§24.4.3](../第五部分-推理系统/第24章-服务化架构与显存类加速.md#2443-prefix-cache命中率模型与三级缓存) 的哈希链断裂)——想绕过失效直接改 KV 的设计都是在制造两份真相;其二,持久化责任单向:应用层对会话状态负全责(复制、备份、TTL),引擎层对 KV 零持久化承诺,实例重启丢 KV 不是事故,丢会话状态才是;其三,监控上"会话可恢复率"与"KV 命中率"是两个层的指标,前者关正确性、后者只关成本与延迟——把后者当可用性指标报警,是层次混淆的信号。
...
本章问题场景为合成案例,其人物、机构与全部数值(P50 44 秒、单轮沙箱准备 8.3 秒等)均为写作构造的示意,登记为 `CLM-028-001`,不构成真实事故记录。预热池容量、尾延迟放大率与会话状态规模三处数字为公式推导估算,输入、公式、假设与误差来源见 `references/claims/chapter-28.yaml` 中 `CLM-028-003`、`CLM-028-004`、`CLM-028-005` 的 `estimate` 字段,读者代入自己的 λ、T_cold、p、N 与轮次分布即可复算。三档隔离对比中的量级数字(微 VM 百毫秒级冷启动、gVisor 类加固的性能损耗等)为业界常见量级表述,目前登记为待补来源(`CLM-028-009`、`CLM-028-010`),使用前请以目标运行时的官方文档与自测数据校准。"Agent Runtime 2.0"一节的协议机制结论(`CLM-028-011`、`CLM-028-012`)挂接 MCP 规范当期版本(具体版本号与复核周期记录在 claim 条目中,正文不锁定版本);审批哈希绑定与权限单调衰减(`CLM-028-013`)是作者的工程原则,规范来源支撑其授权组件化与用户显式同意的方向,但不直接规定这两条机制,按未核验立场登记。本章全部 claim 登记与状态以 `references/claims/chapter-28.yaml` 为准,校验方式为 `npm run docs:check:evidence`。
...
| 术语 | 释义 | 详见 |
|---|---|---|
| Agent 运行时 | 承载"推理—工具调用—代码执行"循环的基础设施层:沙箱、工具通道、权限、状态存储 | §28.1 |
| 占空比(duty cycle) | Agent 任务中 GPU 真正在推理的时间占比(常仅 20–40%);每轮推理应独立经网关进出,不整任务占住并发 | §28.4.1 |
| 微 VM(micro VM) | 硬件虚拟化加精简 guest 内核的轻量虚拟机:百毫秒级冷启动、独立内核买断共享内核风险面——多租户沙箱默认答案 | §28.4.2 |
| WASM 沙箱 | 语言级隔离:毫秒启动、MB 级底噪;原生扩展与子进程多数跑不了,只做高频轻负载补充档 | §28.4.2 |
| gVisor(用户态内核) | 容器加固方案:拦截系统调用缩小风险面,代价是两位数百分比的性能损耗 | §28.5 |
| 预热池容量(λ×T_cold) | 沙箱池的算术:到达率乘冷启动时长再乘峰值余量;隔离技术决定"内存换延迟"的汇率 | §28.4.2 |
| 资源限制(四维齐全) | 沙箱必配的 CPU 配额、内存硬上限、墙钟超时、网络默认拒绝;缺一维就有对应的事故形态 | §28.4.2 |
| 尾延迟放大 | N 轮串行任务至少踩中一次尾部的概率 1−(1−p)^N;单轮超时预算必须从任务级 SLO 反推 | §28.4.3 |
| 幂等键(idempotency key) | 让同一请求重复到达只生效一次的标识;作用域按步而非按会话,重试无幂等键等于重复下单 | §28.4.4 |
| MCP(Model Context Protocol) | 模型与外部世界接口的标准协议:Resources(可读上下文)、Prompts(模板)、Tools(动作)三原语,能力按会话协商 | §28.4.4 |
| Tasks 扩展(长任务) | 超出单次请求生命周期的任务经持久句柄管理:提交即返回句柄,轮询、进度、中途输入、取消与连接解耦 | §28.4.4 |
| Tool Registry / Policy Decision / Credential Broker / Durable State | 运行时四组件:工具注册与信任等级 / 每次调用前的授权判定 / 凭据集中托管与边界注入 / 步级 checkpoint 与事件日志 | §28.4.4 |
| TOCTOU(校验与使用之间) | 审批与执行之间参数被替换的窗口;防御是审批绑定请求参数哈希、执行前校验一致 | §28.4.4 |
| 权限单调衰减 | 委托链上每一跳权限只能收窄不能放大,且由运行时强制——模型行为由输入文本驱动,不能指望"自觉" | §28.4.4 |
| prompt injection / tool poisoning | 同一威胁结构的两个入口:检索内容或工具描述/输出里埋指令改变模型行为;防御是收边界而非"教模型分辨" | §28.4.4 |
| 会话状态(热/温/冷) | 热(活跃,KV 存储)/ 温(可恢复,对象存储)/ 冷(审计回放,归档);沙箱默认无状态,文件显式落对象存储 | §28.4.5 |
